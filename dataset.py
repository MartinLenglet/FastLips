...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(mel_path):
            ...
            # Ignore -inf values
            energy[np.where(energy == -math.inf)] = 0
        ...
        lips_aperture_path = os.path.join(
            self.preprocessed_path,
            "lips_aperture",
            "{}-lips_aperture-{}.npy".format(speaker, basename),
        )
        lips_spreading_path = os.path.join(
            self.preprocessed_path,
            "lips_spreading",
            "{}-lips_spreading-{}.npy".format(speaker, basename),
        )
        if os.path.exists(au_path) and os.path.exists(lips_aperture_path) and os.path.exists(lips_spreading_path):
            au = np.load(au_path)
            lips_aperture = np.load(lips_aperture_path)
            lips_spreading = np.load(lips_spreading_path)
        else:
            au = np.zeros((0, self.au_size))
            lips_aperture = np.zeros(0)
            lips_spreading = np.zeros(0)
        ...
        sample = {
            ...
            "au": au,
            "lips_aperture": lips_aperture,
            "lips_spreading": lips_spreading,
        }
    ...
    def reprocess(self, data, idxs):
        ...
        lips_apertures = [data[idx]["lips_aperture"] for idx in idxs]
        lips_spreadings = [data[idx]["lips_spreading"] for idx in idxs]
        ...
        aus = pad_2D(aus)
        lips_apertures = pad_1D(lips_apertures)
        lips_spreadings = pad_1D(lips_spreadings)
